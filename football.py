#!/usr/bin/env python3
"""
Football AI Demo - Headless version
Saves output to video file instead of displaying windows
Perfect for servers or systems without display
"""
...
class FootballAI:
    ...
    def load_cached_classifier(self, video_path):
        ...
        if os.path.exists(cache_path):
            try:
                ...
                classifier.device = self.config['performance']['device']
                return classifier
    ...
    def train_team_classifier(self, video_path):
        ...
        self.team_classifier = TeamClassifier(device=self.config['performance']['device'])
    ...
    def process_frame(self, frame):
        ...
        if len(players_detections) > 0 and self.team_classifier is not None:
            try:
                players_crops = [sv.crop_image(frame, xyxy) for xyxy in players_detections.xyxy]
                assigned_classes = self.team_classifier.predict(players_crops)
                players_detections.class_id = assigned_classes
            except Exception as e:
    ...
    def create_pitch_view(self, players_detections, referees_detections, ball_detections, transformer):
        ...
        if len(ball_detections) > 0:
            ...
            if self.config['ball_tracking']['enable']:
                if len(pitch_ball_xy) > 0:
                    self.ball_trail.append(pitch_ball_xy[0])
        ...
        if self.config['ball_tracking']['enable'] and len(self.ball_trail) > 1:
            # Convert trail points to proper format
            trail_array = np.array(self.ball_trail)
            if trail_array.ndim == 2 and trail_array.shape[1] == 2:
                # Reshape to expected format
                trail_points = [point.reshape(-1, 2) for point in trail_array]
                trail = np.vstack(trail_points) if trail_points else np.empty((0, 2))
                
                if len(trail) > 0:
                    try:
                        pitch_view = draw_paths_on_pitch(
                            config=self.pitch_config,
                            paths=[trail],  # Pass as single path
                            color=sv.Color.WHITE,
                            pitch=pitch_view
                        )
                    except Exception as e:
                        # Skip ball trail if there's an error
                        pass
    ...
    def run(self, output_path=None):
        ...
        try:
            ...
            out_width = width + 800  # Original + pitch view
            ...
            with tqdm(total=total_frames, desc="Processing frames") as pbar:
                ...
                while True:
                    ...
                    # Process frame
                    annotated_frame, pitch_view = self.process_frame(frame)
                    ...
                    combined_frame[:height, :width] = annotated_frame
                    combined_frame[:600, width:] = pitch_view
                    
                    # Add labels
                    cv2.putText(combined_frame, "Original + Detection", (10, 30),
                               cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                    cv2.putText(combined_frame, "Tactical View", (width + 10, 30),
                               cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                    ...
                    # Optional: save preview image every 100 frames
            ...
            # Generate a final preview image
            preview_path = output_path.replace('.mp4', '_preview_final.jpg')
            cv2.imwrite(preview_path, combined_frame)
...
def main():
    parser = argparse.ArgumentParser(description='Football AI Demo (Headless version)')
